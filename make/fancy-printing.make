...
define PRINT_CMD_DESCRIPTION #<style>(1) <desc0>(2) <prog>(3) <desc1>(4) <fn1>(5) <desc2>(6) <fn2>(7)
	$(HIDE) env printf "$(1)%s$(STYLE_PRGN)%s$(1)%s$(STYLE_FNAME)%s$(1)%s$(STYLE_FNAME)%s$(FP_ENDL)"\
...
define PRINT_SMART_DESC #<prog>
	...
	$(HIDE) $(if $(filter $(PRINTING_GROUP_1),$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_CXX),,$(1),compiling,$<))
	$(HIDE) $(if $(filter $(PRINTING_GROUP_2),$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_CXX),,$(1),compiling,$@))
	$(HIDE) $(if $(filter $(PRINTING_GROUP_3),$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_CP),,,Copying,$@,from,$<))
	$(HIDE) $(if $(filter $(PRINTING_GROUP_4),$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_LD),,,Linking,$@))
	$(HIDE) $(if $(filter $(PRINTING_GROUP_5),$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_CPRESS),,$(1),compressing,$@))
	$(HIDE) $(if $(filter _CONV,$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_CONV),,,Converting,$<,to,$@))
	# generic rule, for anything other (i.e. when <prog> is not empty)
	$(HIDE) $(if $(filter-out "$(PRINTING_GROUP_ALL)",$(1)),$(call PRINT_CMD_DESCRIPTION,$(STYLE_GEN),,$(1),generating,$@),)
	# generic rule, when <prog> is empty
	$(HIDE) $(if $(1),,$(call PRINT_CMD_DESCRIPTION,$(STYLE_GEN),,,Generating,$@))
endef
